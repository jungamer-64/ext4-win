...
/// Explicit ownership phase of one stream-owned oplock control request.
#[derive(Debug)]
enum OplockControlOperationState {
    /// The reactor owns the validated request until its handle lane admits delegation.
    Ready {
        /// Unique top-level completion authority.
        owned: OwnedIrp,
...
/// One handle-serialized oplock FSCTL delegated to the stream-owned FsRtl package exactly once.
#[derive(Debug)]
struct OplockControlOperation {
    /// Current completion-ownership phase.
    state: OplockControlOperationState,
}

impl OplockControlOperation {
    /// Allocates one operation after validating the namespace stream selected by the live request.
    /// # Errors
    ///
...
impl MountedVolumeOperation for OplockControlOperation {
    fn advance_mounted(
        mut self: Box<Self>,
        event: CompletionEvent,
        access: &mut MountedVolumeAccess<'_>,
        ...
        match (state, event) {
            ...
            (
                OplockControlOperationState::Ready { owned, .. },
                OperationEvent::StorageCompleted(_)
                | OperationEvent::DeviceLengthCompleted(_)
                | OperationEvent::RetryElapsed(_)
                | OperationEvent::IntentGranted(_)
                ...
                | OperationEvent::CheckpointGranted(_)
                | OperationEvent::BarrierReleased(_),
            ) => Self::complete(owned, DriverError::InternalInvariantViolation),
            (OplockControlOperationState::Terminal, _) => {
                crate::kernel::fatal::KernelWideInconsistency::completion_reactor_state_corruption()
                    .bugcheck()
            }
        }
...
#[expect(
    unsafe_code,
    reason = "the queued IRP and stable FCB identity move only through the sole reactor"
)]
// SAFETY: The handle admission lane retains the FILE_OBJECT and FCB until this operation either
// completes the IRP or consumes it into the stream-owned FsRtl package.
unsafe impl Send for OplockControlOperation {}
...
enum OplockResume {
    ...
    /// A parent-directory check completed and the remaining namespace plan must advance.
    ContinueNamespaceOplocks {
...
/// Parent-directory mutation authority retained across cleanup deletion re-resolution.
#[derive(Debug)]
struct CleanupParentOplockAuthority {
    /// Exact pending-delete parent covered by the removal check.
    parent: ext4_core::DirectoryNodeId,
    /// Node reservation retained until durable namespace publication.
    _mutation: crate::state::OplockMutationLease,
}

/// Namespace mutation reservations whose required FsRtl checks are not all complete.
#[derive(Debug)]
struct PendingNamespaceOplocks {
    ...
    second: Option<crate::state::OplockMutationLease>,
}

/// Parent-directory oplock authority retained until durable namespace publication.
#[derive(Debug)]
struct NamespaceOplockAuthority {
    ...
    _second: Option<crate::state::OplockMutationLease>,
}

impl PendingNamespaceOplocks {
    /// Converts fully checked reservations into durable-publication authority.
    fn authorize(self) -> NamespaceOplockAuthority {
        NamespaceOplockAuthority {
            plan: self.plan,
            _first: self.first,
            _second: self.second,
        }
...
impl CleanupParentOplockAuthority {
        ...
        match memory::boxed_try_map(
            ...
            |(owned, epoch, crypto, activity, request)| {
                ...
                Self {
                    ...
                    cleanup_parent_oplock: None,
                    namespace_oplocks: None,
        ...
        match request {
            ...
            PreparedMutationRequest::Other(MutationRequestKind::SetInformation) => {
                match crate::request::file_info::set(
                    ...
                    crate::request::file_info::SetFileResolution::CheckNamespaceOplocks(plan) => {
                        Ok(DriverResolveDisposition::CheckNamespaceOplocks { plan })
                    }
        ...
        let resolved = {
            ...
            match Self::execute_resolve(
                ...
                DriverResolveState {
                    ...
                    cleanup_parent_oplock: self.cleanup_parent_oplock.as_ref(),
                    namespace_oplocks: self.namespace_oplocks.as_ref().map(|held| held.plan),
                ...
                Ok(DriverResolveDisposition::CheckNamespaceOplocks { plan }) => {
                    ...
                    let Some(stream) = stream else {
                        return self.continue_namespace_oplocks(
                            owned,
                            pending,
                            plan.second(),
                            operations,
...
impl_mounted_operation_adapter!(OplockControlOperation);
...
/// Allocates one handle-serialized stream oplock control delegation.
/// # Errors
///
/// Returns the still-owned IRP when target validation or allocation fails.
pub(crate) fn oplock_control(
    owned: OwnedIrp,
) -> Result<Box<dyn CompletionOperation>, AdmitOperationError> {
    OplockControlOperation::try_new(owned)
}
